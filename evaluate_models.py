...
if __name__ == '__main__':
    ...
    try:
        ...
        metrics_report['yolov11_gpu1_conf05'] = {
            'mAP50': float(res_old.box.map50),
            'mAP50-95': float(res_old.box.map),
            'precision': float(res_old.box.mp),
            'recall': float(res_old.box.mr),
        }
    ...
    try:
        print('\n' + '='*50)
        print('--- Đánh giá YOLOv11 (runs_compare/runs_compare/train_yolov112/weights/best.pt) ở conf=0.05 ---', flush=True)
        model_new_path = 'runs_compare/runs_compare/train_yolov112/weights/best.pt'
        if not os.path.exists(model_new_path):
            ...
            metrics_report['yolov11_gpu2_conf05'] = {
                'mAP50': float(res_new.box.map50),
                'mAP50-95': float(res_new.box.map),
                'precision': float(res_new.box.mp),
                'recall': float(res_new.box.mr),
            }
